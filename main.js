...
const addReservation= () => {
    ...
    // 1. 선택한 회의(input[type="radio"])의 규모를 출력하는 부분
    const allScale = document.querySelectorAll('.scale'); // 회의 규모 선택을 위한 input 요소들을 모두 선택하여 Nodelist로 할당
    allScale.forEach(element => { // 코드 흐름 2번과 유사하므로 주석 생략
        if(element.checked){
            console.log(`회의 규모: ${element.parentNode.innerText}`);
        }
    });

    // 2. 선택한 장비(input[type="checkbox"])를 출력하는 부분 
    ...
    // 3. 예약자 성함(input[type="text"]) 요소의 현재 값을 가져오는 부분
